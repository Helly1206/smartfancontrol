...
then
	...
	echo "Checking whether pigpio is installed"
	PKG_OK=$(dpkg-query -W --showformat='${Status}\n' pigpio|grep "install ok installed")
	echo Checking for pigpio: $PKG_OK
	if [ "" == "$PKG_OK" ]; then
		echo "No pigpio. Setting up pigpio."
		sudo apt-get --force-yes --yes install pigpio
	fi
